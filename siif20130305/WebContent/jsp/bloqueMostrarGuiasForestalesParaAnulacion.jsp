		...
		<table border="0" class="cuadrado" align="center" width="80%"
			...
			<c:forEach items="${guiasForestales}" var="guia" varStatus="i">
				...
				<tr id="tr<c:out value='${i.count}'></c:out>" class="<%=clase%>"   
					...
					<c:if test="${not guia.anulado}">
						<td>			
							<a href="../../guiaForestal.do?metodo=cargarGuiaForestalParaAnulacionDeGuiaPorNroGuia&idGuia=<c:out value='${guia.id}'></c:out>">
								<bean:message key='SIIF.label.Seleccionar'/>
							</a>
						</td>
					</c:if>
					
					<c:if test="${guia.anulado}">
						<td>
							Guía Anulada			
						</td>
					</c:if>
