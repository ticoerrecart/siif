...
function mostrarDatos(idAforo,idTr){
	...
	$('#divModificacion').load('../../recuperarAforoNuevo.do?metodo=recuperarAforoNuevo&id='+idAforo);
...
function submitir(){
	...
	$('#exitoGrabado').html("");
	$('#errores').html("");
	validarForm("aforoNuevoForm","../aforoNuevo","validarAforoNuevoForm","AforoNuevoForm");
}
...
<table border="0" class="cuadrado" align="center" width="70%"
	...
	<tr>
		...
		<table id="idAforo" border="0" class="cuadrado" align="center"
			...
			<tr>
				<td class="azulAjustado" width="25%"><bean:message key='SIIF.label.TipoDeAforo'/></td>
				<td class="azulAjustado" width="20%"><bean:message key='SIIF.label.Monto'/></td>
				<td class="azulAjustado" width="25%"><bean:message key='SIIF.label.Porcentaje'/></td>
			...
			<c:forEach items="${aforos}" var="aforo" varStatus="i">
				<tr id="tr<c:out value='${i.count}'></c:out>">
					<td class="botonerab">
						<c:out value="${aforo.tipoDeAforo.descripcion}"></c:out>
					</td>
					<td class="botonerab">
						<c:out value="${aforo.monto}"></c:out>
					</td>
					<td class="botonerab"><c:out value="${aforo.porcentaje}"></c:out></td>
					<td>
